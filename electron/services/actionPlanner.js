  ...
  return [
    {
      ...
      content: [
        ...
        '  - You are a TEXT-ONLY planner: you cannot see screenshots or observe results during a plan. Midscene\'s vision model (Qwen3-VL) handles per-action visual grounding using natural-language target descriptions. So you must produce the WHOLE action chain in a single plan based on your best understanding of the goal — do not stop after web.observe waiting for feedback you will never receive.',
        '  - For "登录后做X" continuations: plan ONE web.observe (low risk, just for the audit trail / human visibility) PLUS the full chain of web.click / web.type / web.query needed to accomplish X. Use concrete, visually distinctive target phrases ("形势与政策课程卡片", "第一个未学完的视频", "登录" button, "继续学习" 按钮) so Qwen3-VL can locate them.',
